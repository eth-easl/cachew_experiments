...
def _batch_mean_image_subtraction(image, means, num_channels):
  """Subtracts the given means from each image channel.

  For example:
    means = [123.68, 116.779, 103.939]
    image = _batch_mean_image_subtraction(images, means)
  Note that the rank of `image` must be known.

  Args:
    image: a tensor of size [batch_size, height, width, C].
    means: a C-vector of values to subtract from each channel.
    num_channels: number of color channels in the image that will be distorted.
...
def parse_record(serialized_example):
  ...
  image_cast = tf.cast(image, tf.uint8)
...
def _post_cache_processing(images, labels):
  images = tf.cast(images, tf.float32)
  images = _batch_mean_image_subtraction(images, CHANNEL_MEANS, NUM_CHANNELS)
  return images, labels
  
def post_cache_processing(dataset):
  dataset = dataset.map(_post_cache_processing)
  dataset.prefetch(buffer_size=FLAGS.prefetch_buffer)
  return dataset
