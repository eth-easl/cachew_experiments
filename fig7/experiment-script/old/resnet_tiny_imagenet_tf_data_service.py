...
flags.DEFINE_string('data', None, 'A path with the imagenet data.')
...
flags.DEFINE_integer('shuffle_buffer', 1024, 'The buffer size for shuffle.')ll

flags.DEFINE_integer('dispatcher_port', 5050, 'The port of the tf.data service dispatcher.', lower_bound=0)
...
def _get_filenames():
  ...
  return [
      os.path.join(FLAGS.data, 'train-%05d-of-%05d' % (i, NUM_TRAIN_FILES))
...
def setup_service(dispatcher_port: int)-> (
    tf.data.experimental.service.DispatchServer,
    List[tf.data.experimental.service.WorkerServer]):

    dispatcher = tf.data.experimental.service.DispatchServer(port=dispatcher_port)
    ...
    return dispatcher, [worker1, worker2]


def main(argv):
  del argv

  #dispatcher, workers = setup_service(FLAGS.dispatcher_port)
  ...
  processing_mode="parallel_epochs", service="grpc://146.148.50.233:5050"
  ))
