...
def get_callbacks(pruning_method=None,
  ...
  if FLAGS.enable_tensorboard:
    tensorboard_callback = tf.keras.callbacks.TensorBoard(
        log_dir=FLAGS.model_dir, profile_batch=FLAGS.profile_batch)
...
def define_keras_flags(model=False,
  ...
  flags.DEFINE_string(
      ...
      'the first and last step to profile. For example, "--profile_batch=2,4" '
  ...
  flags.DEFINE_float(
      ...
      default=None,
      help='The target validation accuracy to reach before stopping the training.')
  flags.DEFINE_boolean(
