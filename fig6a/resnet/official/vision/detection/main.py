...
flags.DEFINE_string(
    'nethz',
    default='tamariucai',
    help='NETHZ to identify the TF data dispatcher to connect the local workers to.')

flags.DEFINE_integer(
    'local_workers',
    default=0,
    help='How many local workers should the TF data pipeline use?')
...
def register_local_worker(dispatcher_address):
    ...
    config_proto = service_config_pb2.WorkerConfig(
        ...
        worker_address=defaults.worker_address,  # defaults.worker_address is "localhost"
        port=0,
        protocol='grpc',
        worker_tags=['COLOCATED'],
        heartbeat_interval_ms=100,
        dispatcher_timeout_ms=1000,
        data_transfer_protocol=None,
        data_transfer_address=defaults.worker_address,
        shutdown_quiet_period_ms=0)
    return server_lib.WorkerServer(config_proto, start=True)


def get_dispatcher_location():
    dispatcher_vm_name = os.popen(
        f"gcloud compute instances list --project \"tfdata-service\" --filter=\"name~'{FLAGS.nethz}-cachew-dispatcher-*'\"").read()
    return [s for s in dispatcher_vm_name.split() if s.__contains__(f"{FLAGS.nethz}-cachew-dispatcher-")][
               0] + ":31000"
...
def main(argv):
    ...
    dispatcher_addr = get_dispatcher_location()
    logging.info(f"Found dispatcher running at {dispatcher_addr}", flush=True)

    local_workers = []
    for _ in range(FLAGS.local_workers):
        worker = register_local_worker(dispatcher_addr)
        local_workers.append(worker)  # You must save the worker!
    logging.info(f"Registered {FLAGS.local_workers} local workers.")
