...
class DistributedExecutor(object):
  ...
  def train(
    ...
    if FLAGS.save_checkpoint_freq is not None:
    ...
    while current_step < total_steps:
      ...
      logging.info(f"Training until step {current_step+num_steps}...")
      ...
      if continuous_eval and current_step < total_steps and test_step:
        ...
        eval_metric_result = self._run_evaluation(test_step, current_step,
                                                  eval_metric, eval_iterator)
        logging.info('Step: %s evaluation metric = %s.', current_step,
                     eval_metric_result)
        test_summary_writer(
