...
FLAGS = flags.FLAGS
flags.DEFINE_integer("epoch_count", 4, "The number of epochs")
flags.DEFINE_integer("sleep_time_ms", 150, "The amount of time to sleep per batch")
flags.DEFINE_integer("file_count", 1024, "The name of the job")
flags.DEFINE_string("dispatcher_ip", None, "The alias / IP of the dispatcher")
flags.DEFINE_string("job_name", None, "The name of the job")
...
def _get_filenames(data_path, num_files_to_read):
  ...
  return [
      os.path.join(data_path, 'train/train-%05d-of-01024' % i)
...
def run(params):
  ...
  sleep_time_ms = FLAGS.sleep_time_ms
  dispatcher_ip = FLAGS.dispatcher_ip
  job_name = FLAGS.job_name
  file_count = FLAGS.file_count

  if not dispatcher_ip:
    print("dispatcher_ip must be specified")
    sys.exit()
  ...
  filenames = _get_filenames(data_path, file_count)
  ...
  for epoch in range(epoch_count):
    t = time.time()
    tf.print(f"{t},Currently starting epoch {epoch}", output_stream=sys.stdout)
    for _ in dataset:
      time.sleep(sleep_time_ms / 1000)
    t = time.time()
    tf.print(f"{t},Finished epoch {epoch}", output_stream=sys.stdout)
